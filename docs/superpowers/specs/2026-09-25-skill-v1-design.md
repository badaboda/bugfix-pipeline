...
| 온보딩 | 스택별 템플릿 + 초안 생성(검사를 통과하지 못하는 초안) | 추정하지 않되 첫 진입 비용을 낮춘다 |
...
2. `bp_gate init <slug>` — 작업공간 생성(§6).
3. `bp_gate triage <slug>` — **재서** 트랙을 가른다.

   | 신호 | 재는 법 |
   |---|---|
   | 결정적 재현 | `steps` 가 명령이면 `exec_cmd` 로 **3회**. 세 번의 종료코드가 같고 `observed` 가 매번 재현되면 결정적. 명령이 아니면 비결정 |
   ...
   (이 한 번만 강등 허용, 사용자 승인 필수). 가벼운 → 정식 승급은 §3.3.
...
| `CANNOT-MEASURE` | 정식 트랙에서는 정지하지 않고 **가벼운 트랙으로 넘긴다** — 결정론 판정이 안 서는 버그다. 무엇을 못 쟀는지가 `결정론 판정 없음` 표지의 근거가 된다 |
...
1. 재현 동결은 P0 에서 끝났다.
2. 조사와 수정을 한 번에. 원인은 `파일:라인` 까지, **루프당 가설 하나**.
3. 할 수 있는 만큼 검증하고, 못 한 것에 **표지**를 붙인다.

   | 상황 | 검증 | 표지 |
   |---|---|---|
   | 결정적 재현 + 스위트 있음 | 실패 테스트 → 수정 → `bp_regress` 1회(기준선 캐시) | — |
   | 스위트 없음 | 재현 명령 재생 — 수정 전 빨강 · 수정 후 초록 | `회귀 미검증` |
   | 재현 비결정 · 명령 아님 | 수정 후 재현 시도 N회 + 근거 수집 | `결정론 판정 없음` |

4. `bp_gate light-report <slug>` 가 원인 · diff · 검증 결과 · 표지를 모아 `light_report.md` 를 쓴다.
5. **GATE L** — 사용자 승인 전에는 커밋하지 않는다.
6. P6 과 같다. PR 본문에 **트랙과 표지를 반드시** 적는다(`light-report` 가 본문 초안을 낸다).
...
- 하드 스톱: GATE 1 · GATE 2 · GATE L · 데이터 비가역 작업 · 배포 · cap 도달(`DEFERRED`).
...
| 명령 | 트랙·단계 | 하는 일 | 실패 |
|---|---|---|---|
| `init <slug>` | 공통 P0 | 작업공간 · 무시 경로 확인 · 프로파일 schema 2 검사 · `ledger.json` 뼈대 · `repro.md` 양식 | 2 |
| `triage <slug> [--light]` | 공통 P0 | §3.1 — 재현 3회 · 스위트 유무 · `--light` → 트랙 기록 | 2 |
...
| `light-report <slug>` | 가벼운 | 원인 · diff · 검증 결과 · 표지 → `light_report.md` + PR 본문 초안 | 2 |
...
| `bug-light-fixer` | 가벼운 | `repro.md` · 트랙 기록 | 원인 `파일:라인` · 수정(+ 가능하면 실패 테스트) · 검증 근거 · 표지 · 승급 신호 보고 | 루프당 가설 둘 이상 · 승급 신호를 숨기고 계속 · GATE L 전 커밋 |
...
| 정식 트랙 | 명령으로 결정적 재현 + 자동 테스트 스위트 |
| 가벼운 트랙 | 그 밖 전부 — 표지로 한계를 남긴다 |
...
- Lock-in: RED ≠ GREEN · 채점자는 코드 · 루브릭 작성자 ≠ 채점자 · 귀속은 진리표 · 표본은 전수 · 루프백은 같은
  `cause_id` 안에서만 · cap 은 `CODE` 만 · 같은 우회를 두 번째 쓰면 결함 · 가벼운 트랙은 표지 없이 PR 을 내지 않는다.
...
| 대상 | 방법 |
|---|---|
| `bp_gate.py` | 픽스처 확장(가짜 `exec_cmd` + 125 모드 · sh 명령 루브릭 · patch 로 `R-CONTROL`). 명령 10종 pytest + `--selftest`. 사보타주 대조(행 순서 · 변조 검사 · 125 매핑 · 커밋 감사 · 캐시 무효화 등) |
| 트리아지 | 결정적 · 비결정(3회 중 다름) · 명령 아님 · 스위트 없음 · `--light` |
...
| 계약 린트 `tests/test_contracts.py` | `SKILL.md` · 에이전트 문서의 산출물 파일명·명령이 `bp_gate.py` 상수와 일치하는지 |
...
- **계획 A (코드)** — 프로파일 schema 2 · `bp_profile.py init` · 템플릿 · `bp_gate.py`(트리아지·커밋 감사·기준선
  캐시 포함) · `bp_regress` 캐시 입력 · `bp_ui.py` · `install.sh` 수정
