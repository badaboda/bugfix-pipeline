...
**웹앱**(프론트엔드 · 백엔드) 버그 수정 요청 하나를 받아 **AI 가 고치고 PR 까지** 잇는 오케스트레이터를, 어떤
웹앱 레포에도 설치될 수 있는 형태로 만든다. 스킬 이름은 **`webapp-bugfix`** (2026-09-25 사용자 결정). 버그의
성격에 따라 두 트랙으로 간다.

대상을 웹앱으로 좁힌 뜻: 화면 재현(`repro.sh` + `serve_cmd`)과 P5b 가 1급 경로이고, 스택 템플릿·트리거·문서는
웹앱 레포를 기준으로 쓴다. 웹이 아닌 레포(CLI · 라이브러리)에서도 트랙과 `bp_gate` 는 동작하지만 **지원 대상이
아니다** — 그런 레포를 위해 설계를 넓히지 않는다.
...
| 대상 레포 | 웹앱(프론트엔드 · 백엔드). CLI · 라이브러리 · 모바일·데스크톱 네이티브는 지원 대상 아님 |
| P5b | 웹(DOM) 앱. `ui` 가 없으면 생략(사유 명시) |
| 훅 | 선택. 기존 훅 매니저(husky 등)와 공존 — 설치 거부 시 경고만 |

## 12. `SKILL.md`

- 위치 `skills/webapp-bugfix/SKILL.md`, frontmatter `name: webapp-bugfix`.
