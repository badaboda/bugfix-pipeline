...
- **플러그인 루트** = 이 스킬의 기준 디렉토리(Base directory)의 두 단계 위. `BP=<플러그인 루트>/scripts`.
...
2. **재현 자동화** — `steps` 가 명령이 아니면 `repro.sh` 로 옮긴다: `repro.sh <트리> [<URL>]`, `observed` 를 **출력으로 드러낸다.** 한 번 돌린 출력을 사용자에게 보이고 **「이게 내가 본 것」 확인**을 받는다 — 🛑 하드 스톱. 옮길 수 없으면(외부 결제 · 실기기 · 운영 데이터 전용) 사유를 `repro.md` 에 적는다.
...
4. (선택) `hooks/install.sh install` — `RED:` 규칙을 더 일찍 알려 주는 git 훅. 거부되면 경고만 — 집행은 게이트의 커밋 감사다.
...
| GATE 1 | 🛑 사용자 | 진단 · `fix_scope` · `expected_after` 채우기 · 루브릭 확정 | `bp_gate.py freeze <slug>` |
...
| P4 자기확인 | 같은 구현자 | RED 통과 + 회귀 — 보고일 뿐 | P5 |
| P5 판정 | 게이트 | `bp_gate.py run <slug>` | 아래 표 |
| P5b 스윕 | §4 | 관련 범위 화면 | GATE 2 |
| GATE 2 | 🛑 사용자 | 재현 재생 · 카드 밖 파일 · 루프 N/3 · 원인 교체 이력 · 관찰(미판정) · P5b 생략 사유 | P6 |
...
| 1 `ANCHOR` | P2 — 기준(테스트·루브릭)이 틀렸다. cap 을 쓰지 않는다 |
...
- **기준을 고칠 때** (GATE 1 재진입): 동결 파일을 고치고 `bp_gate.py refreeze <slug> --reason "<사유>"`. 측정 결함으로 직전 `CODE` 를 돌려받으려면 사용자 승인 후 `--refund-last`.
...
2. **승급 신호**(`fix_scope` 2 파일 이상 · 공유 파일 · 첫 가설 실패 · 원인 두 갈래)가 보고되면: 트리아지가 결정적 + 스위트였으면 `bp_gate.py promote <slug> --reason "<신호>"` → 정식 P1. 아니면 승급하지 않고 GATE L 에서 사용자에게 올린다.
...
1. 기준선 사본과 수정 후를 각각 프로파일 `ui.serve_cmd` 로 띄운다 — 두 URL. 끝나면 **둘 다 내린다**(어떤 종료 경로에서든).
   `python3 $BP/bp_ui.py check <트리>` 로 래퍼가 도는지 먼저 확인할 수 있다.
...
4. 확정 회귀마다 `bp_gate.py record-sweep <slug> --regression <작업공간 안 근거 파일>` — 진리표가 `CODE` 를 낸다(cap 에 들어간다). 이후 P3.
...
- 끝나면 `bp_gate.py status <slug> --close done` (버리면 `--close abandoned`). 훅을 설치했으면 `hooks/install.sh uninstall`.
