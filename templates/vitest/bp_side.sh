...
json="$names.json"
# npx 를 쓰지 않는다 — 설치돼 있지 않으면 레지스트리에서 최신판을 몰래 받아 돌린다(최종 리뷰 실측).
"$here/bp_exec.sh" "$tree" -- node_modules/.bin/vitest run --reporter=default --reporter=json --outputFile.json="$json" 2>&1
status=$?
